...
1. 需要创建一个`Listener`类并实现`\Yng\Event\Contracts\EventListenerInterface` 中的`listen`和`process`方法。`listen`
...
```php
$dispatcher = new \Yng\Event\EventDispatcher($listenerProvider);
```
